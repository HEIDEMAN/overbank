...
/**
 * Call the "save" method for the Managed Object Context.
 * @param moc The Managed Object Context to save
 * @return 0 if OK, 1 if save produced an error.
 */
- (int)saveMOC:(NSManagedObjectContext *)moc
{
    // Save the MOC
    NSError *error;
    if (![moc save:&error]) {
        NSLog(@"(saveMOC) Whoops, couldn't save: %@", [error localizedDescription]);
        return 1;
    }
    return 0;
}
...
- (BOOL)saveEntry:(Entry *)line inMOC:(NSManagedObjectContext *)moc
{
    ...
    int errorCode = [self saveMOC:moc];
    if (errorCode == NO)
        NSLog(@"(saveEntry) Saved entry: <%@>", line.concepto);
    else
        NSLog(@"(saveEntry) ERROR: failed to save entry!!");
			  ...
			  managedObjectContext:(NSManagedObjectContext *)managedObjectContext
{
    ...
    NSLog(@"(updateCategoriesInDatabase) ~~~> Updating/Creating the relationship: %@",
          entry.matchingCategory.categoryMatched);
    ...
    @try {
        // Another approach is to disbale notifications
        [dbEntry setPrimitiveValue:dbcategory forKey:@"category"];
        
        // Handling it as NSMutable Set.
        //NSMutableSet *dbEntries = [dbcategory mutableSetValueForKey:@"entries"];
        //[dbEntries addObject:dbEntry];
        
        // Direct way.
        //[dbEntry setValue:dbcategory forKey:@"category"];
    }
    ...
    returnCode = [self saveMOC:managedObjectContext];
...
{
    NSFetchRequest *fetchRequest = [self setEntryFetchRequest:moc];
    NSError *error;
...
{
    NSFetchRequest *request = [self setEntryFetchRequest:moc];
...
/**
 * Searches for entry logs matching the one trying to append to the DB
 */
- (BOOL)matchesExistingCategory:(NSString *)catName managedObjectContext:(NSManagedObjectContext *)managedObjectContext
{
    NSFetchRequest *request = [self setCategoryFetchRequest:managedObjectContext];
    // Form the predicate. Process every field that I've to.
...
/**
 * Sets the Entity to Entry and allocates + initialize a Fetch Request.
 * @param  moc The MOC
 * @return NSFetchRequest created.
 */
...
/**
 * Sets the Entity to Category and allocates + initialize a Fetch Request.
 * @param  moc The MOC
 * @return NSFetchRequest created.
 */
...
- (DBCategory *)findCategory:(NSString *)catName managedObjectContext:(NSManagedObjectContext *)managedObjectContext
{
    ...
    // Setup the string that will contain the query to the Core data.
    NSString *predicate = [NSString stringWithFormat:@"(name LIKE[c] '%@')",catName ];
...
/**
 * Searches for the earlier and later dates stored in the database
 */
- (NSArray *)findDatesInterval:(NSManagedObjectContext *)managedObjectContext
{
    NSFetchRequest *fetchRequest = [self setEntryFetchRequest:managedObjectContext];
    NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"fechaOperacion" ascending:YES];
    NSArray *sortDescriptors = [[NSArray alloc] initWithObjects:sortDescriptor, nil];
    [fetchRequest setSortDescriptors:sortDescriptors];
         ...
         managedObjectContext:(NSManagedObjectContext *)moc
{
    ...
    for (NSString *catName in categoryNames)
    {
        ...
        {
            DBCategory *dbCategory = [[DBCategory alloc] initInMOC:moc];
            [dbCategory setValue:catName forKey:@"name"];
            if ([moc save:nil] == NO)
            {
...
- (NSManagedObject*) entryToDBEntry:(Entry*)line inManagedObjectContext:(NSManagedObjectContext*)moc
{
    ...
    DBEntry *dbEntry = [[DBEntry alloc] initInMOC:moc];
